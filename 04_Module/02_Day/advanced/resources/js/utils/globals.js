export const breakpointResizer = function() {
	const breakpoints = [
		...
		{ name: 'mobile', interval: { min: 575, max: 767}, callbacks: [] },
		{ name: 'tablet', interval: { min: 767, max: 1023 }, callbacks: [] },
		{ name: 'small-desktop', interval: { min: 1023, max: 1279 }, callbacks: [] },
		{ name: 'desktop', interval: { min: 1279, max: 1439 }, callbacks: [] },
		{ name: 'large-desktop', interval: { min: 1439 , max: 1729 }, callbacks: [] },
		{ name: '4k', interval: { min: 1729 , max: 99999 }, callbacks: [] }
	]
	...
	return {
		...
		breakpointDown( breakpointName, func ) {
			...
			if ( find.length && window.matchMedia( `(max-width: ${ find[0].interval.max }px)` ).matches )
				func()
		},
